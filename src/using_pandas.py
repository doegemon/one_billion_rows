...
def process_chunk(chunk):
  agg_data: pd.DataFrame = chunk.groupby("station")["measure"].agg(["min", "max", "mean"]).reset_index()
  return agg_data
...
def create_df(file_path, row_total, chunk_size):
  ...
  with pd.read_csv(file_path, sep=";", header=None, names=["station", "measure"], chunksize=chunk_size) as reader:
  ...
  final_agg_df: pd.DataFrame = final_df.groupby("station").agg({
    "min": "min",
    "max": "max",
    "mean": "mean"
...
if __name__ == "__main__":
  ...
  df: pd.DataFrame = create_df(file_path=data_path, row_total=NUMBER_OF_ROWS, chunk_size=chunk_size)
